...
class MinimumSpanningTreePropertyVerifier : IVerifier<MINIMUMSPANNINGTREE>
{
    public string verifierName { get; } = "Minimum Spanning Tree Property Verifier";
    public string verifierDefinition { get; } = "Verifies that a proposed edge set is a valid minimum spanning tree using the cycle property instead of recomputing the MST.";
    ...
    public bool verify(MINIMUMSPANNINGTREE problem, string solution)
    {
        ...
        var treeAdjacency = BuildTreeAdjacency(nodes, uniqueEdges, graphEdges);
        foreach (var graphEdge in graphEdges)
        {
            var edge = graphEdge.Key;
            int weight = graphEdge.Value;

            if (uniqueEdges.Contains(edge) || edge.u == edge.v)
                continue;

            if (!TryFindMaxEdgeOnPath(edge.u, edge.v, treeAdjacency, new HashSet<string>(), out int maxOnPath))
                return false;
    ...
    private static HashSet<(string u, string v)> ParseCertificateEdges(string certificate)
    {
        ...
        foreach (var pair in edgePairs)
        {
            string u = pair.Key;
            string v = pair.Value;
            uniqueEdges.Add(CanonicalPair(u, v));
        }
    ...
    private static Dictionary<string, List<(string neighbor, int weight)>> BuildTreeAdjacency(
        List<string> nodes,
        HashSet<(string u, string v)> treeEdges,
        Dictionary<(string u, string v), int> weights)
    {
        var adjacency = nodes.ToDictionary(node => node, _ => new List<(string neighbor, int weight)>());
        foreach (var edge in treeEdges)
        {
            int weight = weights[edge];
            adjacency[edge.u].Add((edge.v, weight));
            adjacency[edge.v].Add((edge.u, weight));
        }

        return adjacency;
    }

    private static bool TryFindMaxEdgeOnPath(
        string current,
        string target,
        ...
        out int maxWeight)
    {
        ...
        foreach (var (neighbor, weight) in adjacency[current])
        {
            ...
            if (TryFindMaxEdgeOnPath(neighbor, target, adjacency, visited, out int downstreamMax))
            {
                maxWeight = downstreamMax == int.MinValue ? weight : Math.Max(weight, downstreamMax);
                return true;
            }
        }
