...
class STEINERTREE : IGraphProblem<SteinerTreeBruteForce, SteinerTreeVerifier, SteinerTreeDefaultVisualization, UtilCollectionGraph> {
    ...
    public string inputDescription { get; } = "G, a graph, R, a set of terminal nodes, and k, a weight bound";
    public string outputDescription { get; } = "True or False, whether G has a subtree of weight at most k containing every node in R";
    public string source { get; } = "Karp, Richard M. Reducibility among combinatorial problems. Complexity of computer computations. Springer, Boston, MA, 1972. 85-103.";
    public string sourceLink { get; } = "https://cgi.di.uoa.gr/~sgk/teaching/grad/handouts/karp.pdf";
    public const string InstanceGrammar = "{((N,E),R,K) | N is set, E subset N unorderedcross N, R is set, K is int}";
    ...
    public string certificateFormat { get; } =
        $"Format: {SteinerTreeVerifier.CertificateGrammar} Example: {SteinerTreeVerifier.CertificateExample}";
    ...
    public STEINERTREE(string GInput) {
        ...
        StringParser steinerTreeGraph = new(InstanceGrammar);
